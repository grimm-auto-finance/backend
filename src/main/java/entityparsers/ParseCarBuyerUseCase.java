...
public class ParseCarBuyerUseCase {
  ...
  public CarBuyer parse() throws Exceptions.ParseException {
    ...
    int creditScore;
    try {
      // TODO: decide whether these should expect a complete attributmap (with multiple objects) or
      // just the buyer part (where does the extraction occur?)
      ...
      creditScore =
          (int)
              Math.round((Double) buyerMap.getItem(EntityStringNames.BUYER_CREDIT).getAttribute());
